...
class ListingStacker(Stacker):
    ...
    def derive_datasets(self, input_dataset_dict, stack_output_info, tile_type_info):
        ...
        if 'NBAR' in input_dataset_dict:
            ds_info = input_dataset_dict['NBAR']
            logger.info("NBAR: for daterange (%s -> %s). Path: %s", ds_info['start_datetime'], ds_info['end_datetime'], ds_info['tile_pathname'])
        else: 
            logger.info("No NBAR data for daterange (%s -> %s)",  stack_output_info['start_datetime'], stack_output_info['end_datetime'])
